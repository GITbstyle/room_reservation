...
class AddRoomView(View):
    ...
    def post(self, request):
        ...
        if ConferenceRoom.objects.filter(name=name).first():
            return render(request, "add_room.html", context={"error": "Sala o podanej nazwie już istnieje, zmień nazwę i spróbuj ponownie"})
...
class ModifyRoomView(View):
    ...
    def post(self, request, room_id):
        ...
        if capacity <= 0:
            return render(request, "modify_room.html", context={"room": room,
                                                                                                "error": "Pojemność sali musi być dodarnia"})
...
class ReservationView(View):
    ...
    def post(self, request, room_id):
        ...
        if RoomReservation.objects.filter(room_id_id=room, date=date):
...
class RoomDetailsView(View):
    def get(self, request, room_id):
        room = ConferenceRoom.objects.get(id=room_id)
        reservations = room.roomreservation_set.filter(date__gte=str(datetime.date.today())).order_by('date')
        return render(request, "room_details.html", context={"room": room, "reservation": reservations})
